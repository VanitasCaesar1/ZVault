...
impl TokenStore {
    ...
    ///
    /// Used during vault initialization to store the root token generated by
    /// the seal manager. The plaintext token is returned as-is (it was already
    /// generated elsewhere and shown to the operator).
    ///
    /// # Errors
    ///
    /// Returns [`TokenError::Barrier`] if storage fails.
    pub async fn create_with_token(
        ...
        info!(display_name = %entry.display_name, "token created (pre-generated)");
    ...
    /// Expired tokens are included — the caller can filter them.
    ///
    /// # Errors
    ///
    /// Returns [`TokenError::Barrier`] if storage fails.
    ...
    /// Count the number of stored tokens.
    ///
    /// # Errors
    ///
    /// Returns [`TokenError::Barrier`] if storage fails.
    pub async fn count(&self) -> Result<usize, TokenError> {
        let keys = self.barrier.list(TOKEN_PREFIX).await?;
        Ok(keys.len())
    }
